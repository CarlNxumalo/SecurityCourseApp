<script lang="ts">
	...
	let courseLinks = data.courses.reduce((acc, course) => {
		acc[course.name] = `/dashboard/module/${course.courseID}`;
		return acc;
	}, {});
	
	let posts = [
		{ name: 'Overview', icon: ChartPieOutline, href: '/dashboard' },
		{
			name: 'Courses',
			icon: TableColumnSolid,
			children: courseLinks
		},
		{ name: 'Invoices', icon: CogOutline, href: '/dashboard/invoice' },
		{ name: 'Registration', icon: BookSolid, href: '/dashboard/register-modules' },
		{
			name: 'Authentication',
			icon: LockSolid,
			children: {
